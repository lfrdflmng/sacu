...
<head>
    <meta charset="utf-8">
    <title>SACU - Inicio de Sesión</title>
    ...
    <link type="text/css" href="{!! URL::asset('css/login.css') !!}" rel="stylesheet">
    ...
    <div class="login-cont">
        <div class="login">
            <div class="login__check">
                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" viewBox="0 0 294.812 294.812" style="enable-background:new 0 0 294.812 294.812;" xml:space="preserve" width="512px" height="512px">
            ...
            <div class="login__form">
                <form id="frm_login" method="post" action="inicio-post">
                    <div class="login__row">
                        <svg class="login__icon name svg-icon" viewBox="0 0 20 20" style="visibility: hidden;">
                            <path d="M0,20 a10,8 0 0,1 20,0z M10,0 a4,4 0 0,1 0,8 a4,4 0 0,1 0,-8" />
                        </svg>
                        <input type="text" class="login__input name" name="nombre_usuario" placeholder="Usuario" autofocus>
                    </div>
                    <div class="login__row">
                        <svg class="login__icon pass svg-icon" viewBox="0 0 20 20" style="visibility: hidden;">
                            <path d="M0,20 20,20 20,8 0,8z M10,13 10,16z M4,8 a6,8 0 0,1 12,0" />
                        </svg>
                        <input type="password" class="login__input pass" name="contrasena" placeholder="Contraseña">
                    </div>
                    <button type="submit" class="login__submit">Entrar</button>
                    <!--p class="login__signup">¿No tiene cuenta? &nbsp;<a>Registrarse</a></p-->
                    {!! csrf_field() !!}
                </form>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    function ingresar() {
        ...
        enviarFormulario($frm, function(data) {
            if (typeof data['ok'] !== 'undefined' && data['ok'] /*&& typeof data['token'] === 'string' && data['token'].length*/) {
                //TODO: retornar la url de 'intended page' para redireccionar a esa url
                aprobar();
            }
            else {
                rechazar();
    ...
    /**
     * Avisa al usuario de nombre o clave incorrecta
     */
    function rechazar() {
        const $modal = $('.login-cont');
        const $frm = $modal.find('#frm_login');
        const $campo_nombre = $frm.find('.login__input.name');

        $frm.removeClass('processing');
        agregarClaseTemp($modal, 'animated shake');
        $campo_nombre.focus();
    }
     ...
     * Acciones
     */
    function atarAcciones() {
        const $frm = $('#frm_login');

        //Si se presiona Enter y no se ha ingresado la contraseña, se selecciona el campo
        $frm.find('input.name').keypress(function(e) {
            if(e.which === 13) {
                const $campo_contra = $frm.find('input.pass');
                if (!$campo_contra.val().length) {
                    $campo_contra.focus();
                    e.preventDefault();
                    e.stopPropagation();
                    return false;
